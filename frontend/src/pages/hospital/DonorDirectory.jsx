...
const DonorDirectory = () => {
  ...
  const fetchDonors = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `https://blood-bank-management-system-6e0d.onrender.com/api/hospital/donors?${queryParams}`,
  ...
  const logContactAttempt = async (donorId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `https://blood-bank-management-system-6e0d.onrender.com/api/hopital/donors/${donorId}/contact`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
    } catch (err) {
